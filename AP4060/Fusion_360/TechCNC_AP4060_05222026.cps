...
longDescription = "https://techcnc.ca";
...
function onOpen() {
  ...
  writeBlock(gAbsIncModal.format(0), gFeedModeModal.format(90), gFormat.format(94), gFormat.format(17), gFormat.format(40), gPlaneModal.format(80));
...
function onSection() {
  ...
  var opTime = currentSection.getCycleTime();
  if (opTime > 0) {
    writeComment(localize("Operation time") + ": " + formatTime(opTime));
  }
  ...
  // Повышенное кол-ва масла MQL (30 ml/h) для сверления, развертывания и T-slot фрезы
  var highMQL = (
    ...
    tool.type == TOOL_MILLING_SLOT
  );
  var qMist = highMQL ? 30 : 5;
  var qFloodMist = highMQL ? 30 : 10;
  var qFlood = highMQL ? 30 : 20;
  switch (coolant) {
    case COOLANT_OFF:
      writeBlock("M9"); // выключить охлаждение
      break;
    case COOLANT_AIR:
      // Только воздух — MQL выключен
    ...
    case COOLANT_MIST:
      ...
      writeComment("MQL oil amount Q" + qMist + " ml/h");
      writeBlock("G10", "L80", "P8133", "Q" + qMist);
      writeBlock("M7");
      writeBlock("M8");
      break;
    case COOLANT_FLOOD_MIST:
      ...
      writeComment("MQL oil amount Q" + qFloodMist + " ml/h");
      writeBlock("G10", "L80", "P8133", "Q" + qFloodMist);
      writeBlock("M7");
      writeBlock("M8");
      break;
    case COOLANT_FLOOD:
      // Заливное (воздух + туман на базе 20)
      writeComment("Coolant: Flood");
      writeComment("MQL oil amount Q" + qFlood + " ml/h");
      writeBlock("G10", "L80", "P8133", "Q" + qFlood);
      writeBlock("M7");
      writeBlock("M8");
      break;
    default:
      writeComment("Unsupported coolant type: " + coolant);
  }
}
  ...
  //setCoolant(tool.coolant); // writes the required coolant codes
  //writeBlock ("G04 P5"); //time delay after M8
...
function onCyclePoint(x, y, z) {
  ...
  if (isFirstCyclePoint()) {
    ...
    case "chip-breaking":
      ...
      break;
    case "deep-drilling":
      writeComment("MQL oil amount Q30 ml/h");
      writeBlock("G10", "L80", "P8133", "Q30"); //увеличиваем кол-ва масла в систему MQL
      if (P > 0) {
        expandCyclePoint(x, y, z);
      } else {
        writeBlock(
...
function onClose() {
  ...
  writeRetract(Z); //writeRetract(Z);
  writeBlock("G0 G53 X-50 Y-380"); // moving to G53 X-70 Y-380 only for AP4060
...
function writeRetract() {
  ...
  if (retract && retract.words.length > 0) {
    ...
    case "G28":
      ...
      writeBlock(gFormat.format(0), gAbsIncModal.format(53), retract.words);
